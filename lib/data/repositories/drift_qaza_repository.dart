...
class DriftQazaRepository implements QazaRepository {
  ...
  Future<List<QazaRecord>> getRecords({required String userId, PrayerType? prayerType, QazaStatus? status}) async {
    ...
    String? afterId;
    while (true) {
      final page = await getPage(userId: userId, limit: 500, prayerType: prayerType, status: status, afterOriginalDate: afterDate, afterId: afterId);
  ...
  Future<QazaPage> getPage({required String userId, int limit = 50, PrayerType? prayerType, QazaStatus? status, DateTime? afterOriginalDate, String? afterId}) async {
    final page = await database.qazaRecordsDao.getKeysetPage(userId: userId, limit: limit, prayerType: prayerType?.name, status: status?.name, afterOriginalDate: afterOriginalDate, afterId: afterId);
    return QazaPage(records: page.records, hasMore: page.hasMore);
  }

  @override
  Future<QazaRecord?> getOldestPending({required String userId, required PrayerType prayerType}) => database.qazaRecordsDao.getOldestPending(userId: userId, prayerType: prayerType.name).then((row) => row);
  ...
  Future<QazaHistoryPage> getHistoryPage({required String userId, int limit = 50, PrayerType? prayerType, QazaStatus? status = QazaStatus.completed, DateTime? from, DateTime? to, DateTime? beforeOriginalDate, String? beforeId}) async {
    final page = await database.qazaRecordsDao.getHistoryPage(userId: userId, limit: limit, prayerType: prayerType?.name, status: status?.name, from: from, to: to, beforeOriginalDate: beforeOriginalDate, beforeId: beforeId);
  ...
  Future<QazaProgressSummary> getProgressSummary({required String userId}) async {
    ...
    for (final entry in counts.entries) { pending[entry.key] = entry.value[QazaStatus.pending] ?? 0; completed[entry.key] = entry.value[QazaStatus.completed] ?? 0; }
    return QazaProgressSummary(overall: QazaProgress(pending: pending.values.fold(0, (a, b) => a + b), completed: completed.values.fold(0, (a, b) => a + b)), byPrayer: {for (final prayer in PrayerType.values) prayer: PrayerProgress(prayerType: prayer, progress: QazaProgress(pending: pending[prayer]!, completed: completed[prayer]!))});
  }
  ...
  Future<void> completeRecords({required String userId, required List<String> recordIds, required DateTime completedAt}) async {
    ...
    await database.transaction(() async {
      for (final id in recordIds.toSet()) {
        final existing = await database.qazaRecordsDao.findById(userId: userId, id: id);
        if (existing == null || existing.status == QazaStatus.completed) continue;
        await database.qazaRecordsDao.updateRecord(existing.copyWith(status: QazaStatus.completed, completedAt: completedAt, updatedAt: completedAt));
      }
    });
  }

  QazaRecordsCompanion _toCompanion(QazaRecord r) => QazaRecordsCompanion.insert(id: r.id, userId: r.userId, prayerType: r.prayerType.name, originalDate: r.originalDate, status: r.status.name, completedAt: r.completedAt == null ? const Value.absent() : Value(r.completedAt), createdAt: r.createdAt, updatedAt: r.updatedAt);
}
