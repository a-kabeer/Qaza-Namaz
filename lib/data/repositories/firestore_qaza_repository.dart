...
class FirestoreQazaRepository implements QazaRepository {
  ...
  Future<List<QazaRecord>> getRecords({required String userId, PrayerType? prayerType, QazaStatus? status}) async {
    ...
    final records = (await query.get()).docs.map(_fromDocument).toList()..sort((a, b) => a.originalDate.compareTo(b.originalDate));
    return records;
  }
  ...
  Future<QazaPage> getPage({required String userId, int limit = 50, PrayerType? prayerType, QazaStatus? status, DateTime? afterOriginalDate, String? afterId}) async {
    ...
    if (afterOriginalDate != null) query = query.startAfter([QazaDate.key(QazaDate.normalize(afterOriginalDate)), afterId]);
    final snapshot = await query.limit(limit + 1).get();
    final hasMore = snapshot.docs.length > limit;
    final docs = hasMore ? snapshot.docs.take(limit) : snapshot.docs;
    return QazaPage(records: docs.map(_fromDocument).toList(growable: false), hasMore: hasMore);
  }
  ...
  Future<QazaHistoryPage> getHistoryPage({required String userId, int limit = 50, PrayerType? prayerType, QazaStatus? status = QazaStatus.completed, DateTime? from, DateTime? to, DateTime? beforeOriginalDate, String? beforeId}) async {
    ...
    if (prayerType != null) query = query.where('prayerType', isEqualTo: prayerType.name);
    if (status != null) query = query.where('status', isEqualTo: status.name);
    if (from != null) query = query.where('originalDate', isGreaterThanOrEqualTo: QazaDate.key(QazaDate.normalize(from)));
    if (to != null) query = query.where('originalDate', isLessThanOrEqualTo: QazaDate.key(QazaDate.normalize(to)));
    query = query.orderBy('originalDate', descending: true).orderBy(FieldPath.documentId, descending: true);
    if (beforeOriginalDate != null) query = query.startAfter([QazaDate.key(QazaDate.normalize(beforeOriginalDate)), beforeId]);
    final snapshot = await query.limit(limit + 1).get();
    final hasMore = snapshot.docs.length > limit;
    final docs = hasMore ? snapshot.docs.take(limit) : snapshot.docs;
    return QazaHistoryPage(records: docs.map(_fromDocument).toList(growable: false), hasMore: hasMore);
  }

  @override
  Future<QazaProgressSummary> getProgressSummary({required String userId}) async => QazaProgressSummary.fromRecords(await getRecords(userId: userId));

  @override
  Future<void> addRecord(QazaRecord record) async {
    final reference = _recordsCollection(record.userId).doc(record.id);
    await _firestore.runTransaction((transaction) async { final existing = await transaction.get(reference); if (!existing.exists) transaction.set(reference, _toMap(record)); });
  }
  ...
  Future<void> completeRecords({required String userId, required List<String> recordIds, required DateTime completedAt}) async {
    for (final recordId in recordIds.toSet()) {
      ...
      await _firestore.runTransaction((transaction) async {
        ...
        if (record.status == QazaStatus.completed && existingCompletedAt != null && !completedAt.isBefore(existingCompletedAt)) return;
  ...
  QazaRecord _fromDocument(DocumentSnapshot<Map<String, dynamic>> document) {
    ...
    final prayerType = PrayerType.values.firstWhere((value) => value.name == data['prayerType'], orElse: () => throw StateError('Unknown prayer type in record ${document.id}.'));
    final status = QazaStatus.values.firstWhere((value) => value.name == data['status'], orElse: () => throw StateError('Unknown Qaza status in record ${document.id}.'));
  ...
  DateTime _originalDate(Object? value, String id) { if (value is String) return QazaDate.parseKey(value); if (value is Timestamp) return QazaDate.fromRecordId(id); throw StateError('Missing or invalid originalDate in Firestore Qaza record.'); }
  DateTime _timestamp(Object? value, String field) { if (value is Timestamp) return value.toDate(); throw StateError('Missing or invalid $field in Firestore Qaza record.'); }
