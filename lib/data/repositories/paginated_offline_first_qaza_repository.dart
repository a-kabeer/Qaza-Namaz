...
class PaginatedOfflineFirstQazaRepository extends OfflineFirstQazaRepository {
  PaginatedOfflineFirstQazaRepository({required QazaRepository remote, required QazaLocalStore localStore, Stream<bool>? connectivityChanges, DateTime Function()? now})
  ...
  @override
  Future<QazaHistoryPage> getHistoryPage({required String userId, PrayerType? prayerType, QazaStatus? status, DateTime? originalDateFrom, DateTime? originalDateTo, String? cursor, int limit = 25, bool ascending = false}) {
    if (userId != activeUserId) return Future.value(const QazaHistoryPage(records: []));
    return _historyStore.getHistoryPage(userId: userId, prayerType: prayerType, status: status, originalDateFrom: originalDateFrom, originalDateTo: originalDateTo, cursor: cursor, limit: limit, ascending: ascending);
  }

  Future<void> close() async {
    dispose();
    if (_historyStore is SqliteQazaLocalStore) await (_historyStore as SqliteQazaLocalStore).close();
  }
}
