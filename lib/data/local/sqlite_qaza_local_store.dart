...
class SqliteQazaLocalStore implements QazaLocalStore {
  ...
  Future<Database> _db() async {
    ...
    _database = await openDatabase(p.join(databasesPath, databaseName), version: _version, onCreate: (db, _) => _createSchema(db), onUpgrade: (db, oldVersion, newVersion) async {
      if (oldVersion < 2) await db.execute('CREATE TABLE IF NOT EXISTS qaza_meta (name TEXT PRIMARY KEY, value TEXT NOT NULL)');
    });
    ...
    try {
      ...
      await db.transaction((txn) async {
        ...
        for (final entry in sync.entries) {
          ...
          if (value is String && value.isNotEmpty) await txn.insert('qaza_sync_meta', {'user_id': entry.key, 'last_sync': value}, conflictAlgorithm: ConflictAlgorithm.replace);
        }
  ...
  Future<OfflineCacheSnapshot> load() async {
    ...
    for (final row in rows) { final record = _recordFromRow(row); records.putIfAbsent(record.userId, () => []).add(record); }
    final outboxRows = await db.query('qaza_outbox', orderBy: 'id ASC');
    final outbox = <String, List<PendingSyncOp>>{};
    for (final row in outboxRows) { final op = _opFromRow(row); outbox.putIfAbsent(op.userId, () => []).add(op); }
  ...
  @override
  Future<int> getPendingSyncCount(String userId) async { final db = await _db(); final rows = await db.rawQuery('SELECT COUNT(*) AS count FROM qaza_outbox WHERE user_id = ?', [userId]); return (rows.first['count'] as num).toInt(); }
  ...
  @override
  Future<void> deletePendingSyncOp(String userId, String opId) async { final db = await _db(); await db.delete('qaza_outbox', where: 'user_id = ? AND id = ?', whereArgs: [userId, opId]); }
  ...
  Future<QazaLedgerSummary> getSummary(String userId) async {
    ...
    final pendingByPrayer = <PrayerType, int>{}, completedByPrayer = <PrayerType, int>{};
  ...
  @override
  Future<void> saveRecords(String userId, List<QazaRecord> records) async { final db = await _db(); await db.transaction((txn) async { for (final record in records.where((r) => r.userId == userId)) await txn.insert('qaza_records', _recordValues(record), conflictAlgorithm: ConflictAlgorithm.replace); }); }

  @override
  Future<void> saveOutbox(String userId, List<PendingSyncOp> ops) async { final db = await _db(); await db.transaction((txn) async { for (final op in ops.where((o) => o.userId == userId)) await txn.insert('qaza_outbox', {'id': op.id, 'user_id': userId, 'payload': jsonEncode(op.toJson())}, conflictAlgorithm: ConflictAlgorithm.replace); }); }

  @override
  Future<void> saveLastSync(String userId, DateTime? lastSync) async { final db = await _db(); if (lastSync == null) await db.delete('qaza_sync_meta', where: 'user_id = ?', whereArgs: [userId]); else await db.insert('qaza_sync_meta', {'user_id': userId, 'last_sync': lastSync.toIso8601String()}, conflictAlgorithm: ConflictAlgorithm.replace); }
  ...
  Future<QazaHistoryPage> getHistoryPage({required String userId, PrayerType? prayerType, QazaStatus? status, DateTime? originalDateFrom, DateTime? originalDateTo, String? cursor, int limit = 25, bool ascending = false}) async {
    ...
    final db = await _db();
    final where = <String>['user_id = ?']; final args = <Object?>[userId];
    if (prayerType != null) { where.add('prayer_type = ?'); args.add(prayerType.name); }
    if (status != null) { where.add('status = ?'); args.add(status.name); }
    ...
    if (cursor != null) {
      ...
      if (cursorRow.isNotEmpty) { final date = cursorRow.first['original_date']! as String; final id = cursorRow.first['id']! as String; where.add(ascending ? '(original_date > ? OR (original_date = ? AND id > ?))' : '(original_date < ? OR (original_date = ? AND id < ?))'); args.addAll([date, date, id]); }
    }
    final rows = await db.query('qaza_records', where: where.join(' AND '), whereArgs: args, orderBy: ascending ? 'original_date ASC, id ASC' : 'original_date DESC, id DESC', limit: '${limit + 1}');
    final hasMore = rows.length > limit; final pageRows = hasMore ? rows.take(limit).toList() : rows; final records = pageRows.map(_recordFromRow).toList(growable: false);
