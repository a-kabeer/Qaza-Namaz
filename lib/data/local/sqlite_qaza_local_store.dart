...
class SqliteQazaLocalStore implements QazaLocalStore {
  ...
  Future<Database> _db() async {
    ...
    _database = await openDatabase(
      ...
      onUpgrade: (db, oldVersion, newVersion) async {
        if (oldVersion < 2) {
          await db.execute(
            'CREATE TABLE IF NOT EXISTS qaza_meta (name TEXT PRIMARY KEY, value TEXT NOT NULL)',
          );
        }
      },
    );
  ...
  Future<void> _createSchema(Database db) async {
    await db.execute(
      'CREATE TABLE qaza_meta (name TEXT PRIMARY KEY, value TEXT NOT NULL)',
    );
    ...
    await db.execute(
      'CREATE INDEX idx_qaza_user_date ON qaza_records(user_id, original_date DESC, id DESC)',
    );
    await db.execute(
      'CREATE INDEX idx_qaza_user_prayer ON qaza_records(user_id, prayer_type, original_date DESC, id DESC)',
    );
    await db.execute(
      'CREATE INDEX idx_qaza_user_status ON qaza_records(user_id, status, original_date DESC, id DESC)',
    );
    await db.execute(
      'CREATE TABLE qaza_outbox (id TEXT PRIMARY KEY, user_id TEXT NOT NULL, payload TEXT NOT NULL)',
    );
    await db.execute('CREATE INDEX idx_qaza_outbox_user ON qaza_outbox(user_id)');
    await db.execute(
      'CREATE TABLE qaza_sync_meta (user_id TEXT PRIMARY KEY, last_sync TEXT)',
    );
  }
  ...
  Future<void> _migrateLegacyIfNeeded(Database db) async {
    ...
    if (raw == null || raw.isEmpty) {
      await db.insert(
        'qaza_meta',
        {'name': _migrationName, 'value': 'empty'},
      ...
      await db.transaction((txn) async {
        ...
        for (final entry in sync.entries) {
          ...
          if (value is String && value.isNotEmpty) {
            await txn.insert(
              'qaza_sync_meta',
              {'user_id': entry.key, 'last_sync': value},
              conflictAlgorithm: ConflictAlgorithm.replace,
            );
          }
        }
        await txn.insert(
          'qaza_meta',
          {'name': _migrationName, 'value': 'complete'},
          conflictAlgorithm: ConflictAlgorithm.replace,
        );
      });
    } catch (_) {
      // Leave the marker unset so a transient/corrupt legacy payload cannot
      // silently become a permanent data-loss state. The next open retries it.
      rethrow;
    }
  }
  ...
  Future<OfflineCacheSnapshot> load() async {
    ...
    for (final row in outboxRows) {
      final op = PendingSyncOp.fromJson(
  ...
  Future<QazaLedgerSummary> getSummary(String userId) async {
    ...
    final statusRows = await db.rawQuery(
      'SELECT status, COUNT(*) AS count FROM qaza_records WHERE user_id = ? GROUP BY status',
      [userId],
    );
    var pending = 0;
    var completed = 0;
    ...
    final byPrayerRows = await db.rawQuery(
      'SELECT prayer_type, status, COUNT(*) AS count FROM qaza_records WHERE user_id = ? GROUP BY prayer_type, status',
      [userId],
    );
    ...
    final byPrayer = <PrayerType, QazaProgress>{
      for (final prayer in PrayerType.values)
        prayer: QazaProgress(
          pending: pendingByPrayer[prayer] ?? 0,
          completed: completedByPrayer[prayer] ?? 0,
        ),
    };
    return QazaLedgerSummary(
  ...
  Future<void> saveLastSync(String userId, DateTime? lastSync) async {
    ...
    if (lastSync == null) {
      ...
      await db.insert(
        'qaza_sync_meta',
        {'user_id': userId, 'last_sync': lastSync.toIso8601String()},
        conflictAlgorithm: ConflictAlgorithm.replace,
      );
    }
  }
  ...
  Future<QazaHistoryPage> getHistoryPage({
    ...
    if (prayerType != null) {
      where.add('prayer_type = ?');
      args.add(prayerType.name);
    }
    if (status != null) {
      where.add('status = ?');
    ...
    if (originalDateFrom != null) {
      where.add('original_date >= ?');
      args.add(_dateKey(originalDateFrom));
    }
    if (originalDateTo != null) {
      where.add('original_date <= ?');
      args.add(_dateKey(originalDateTo));
    }
    if (cursor != null) {
      final cursorRow = await db.query(
        'qaza_records',
        columns: ['original_date', 'id'],
        where: 'id = ? AND user_id = ?',
        whereArgs: [cursor, userId],
        limit: '1',
      );
      if (cursorRow.isNotEmpty) {
        ...
        if (ascending) {
          where.add('(original_date > ? OR (original_date = ? AND id > ?))');
        } else {
          where.add('(original_date < ? OR (original_date = ? AND id < ?))');
        }
