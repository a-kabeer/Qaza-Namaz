...
class _HistoryPageState extends State<HistoryPage> {
  ...
  Future<void> _load() async {
    try {
      final records = await _service.history(widget.userId);
      if (mounted) {
        setState(() {
          _records = records;
          _loading = false;
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                'Completed Qaza prayers will appear here as a permanent ledger.',
        ...
        children: [
          ...
          for (final entry in grouped.entries) ...[
            Padding(
              ...
              child: Text(_formatGroup(entry.key), style: Theme.of(context).textTheme.titleMedium),
            ),
            for (final record in entry.value)
              Card(
                child: ListTile(
                  leading: Icon(Icons.check_circle, color: Theme.of(context).colorScheme.tertiary),
                  title: Text(record.prayerType.label),
                  subtitle: Text('Original date: ${_formatDate(record.originalDate)}'),
                  trailing: Text(record.completedAt == null ? 'Completed' : _formatTime(record.completedAt!)),
                ),
              ),
