...
class _AddQazaScreenState extends ConsumerState<AddQazaScreen> {
  ...
  void _openDateStep() => setState(() => step = 1);
  ...
  Future<void> _togglePrayer(PrayerType prayer, bool selected) async {
    ...
    if (prayers.isEmpty) {
      setState(() { existingCombinations = 0; newCombinations = 0; });
      return;
    }
  ...
  Future<void> _confirmAndSave() async {
    if (prayers.isEmpty || saving || newCombinations <= 0) return;
    ...
    try {
      ...
      await showDialog<void>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text('$before independent Qaza record${before == 1 ? '' : 's'} were added to your ledger.'),
          actions: [FilledButton(onPressed: () => Navigator.pop(context), child: const Text('Done'))],
        ),
      );
      if (mounted) Navigator.pop(context, before);
    } finally {
  ...
  Widget _modeStep() => ListView(
    key: const ValueKey('qaza_step_list_0'), padding: const EdgeInsets.all(20), children: [
      ...
      FilledButton.icon(key: const Key('qaza_continue_button'), onPressed: _openDateStep, icon: const Icon(Icons.arrow_forward_rounded), label: const Text('Continue')),
    ]);

  Widget _dateStep() {
    final state = ref.watch(calendarControllerProvider);
    return ListView(key: const ValueKey('qaza_step_list_1'), padding: const EdgeInsets.all(20), children: [
      ...
      const SizedBox(height: 16),
      FilledButton.icon(onPressed: dates.isEmpty ? null : _continueFromDates, icon: const Icon(Icons.arrow_forward_rounded), label: const Text('Next: Choose missed prayers')),
    ]);
  }
  ...
  Widget _prayerStep() {
    ...
    return ListView(key: const ValueKey('qaza_step_list_2'), padding: const EdgeInsets.fromLTRB(20, 12, 20, 28), children: [
      ...
      for (final prayer in PrayerType.values)
        Card(margin: const EdgeInsets.only(bottom: 8), child: CheckboxListTile(
          ...
          title: Text(prayer.label), subtitle: Text(prayer.rakats), controlAffinity: ListTileControlAffinity.trailing,
        )),
      const SizedBox(height: 8),
      ...
      const SizedBox(height: 16),
      FilledButton.icon(onPressed: prayers.isEmpty || newCombinations <= 0 || saving || checking ? null : _confirmAndSave, icon: Icon(saving ? Icons.hourglass_top_rounded : Icons.verified_rounded), label: Text(saving ? 'Creating Records...' : 'Review & Create Records')),
    ]);
  }
}
