...
class _AddQazaScreenState extends ConsumerState<AddQazaScreen> {
  ...
  @override
  void initState() { super.initState(); WidgetsBinding.instance.addPostFrameCallback((_) => _checkExisting()); }

  QazaAvailabilityAnalysis _analysisFor(Iterable<PrayerType> prayerTypes) {
    final userId = ref.read(activeUserIdProvider);
    if (userId == null) return QazaAvailabilityAnalysis(total: dates.length * prayerTypes.length, alreadyRecorded: 0, alreadyPrayed: 0, newCount: dates.length * prayerTypes.length, candidates: const [], newCandidates: const []);
    return _availability.analyze(userId: userId, dates: dates, prayerTypes: prayerTypes, existingRecords: existing);
  }
  ...
  Future<void> _continueFromDates() async {
    ...
    if (!mounted) return;
    final userId = ref.read(activeUserIdProvider);
    if (userId != null && !dates.any((date) => _availability.isDateAvailable(userId: userId, date: date, existingRecords: existing))) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('No eligible prayers remain for the selected date(s).'))); return;
    }
    setState(() => step = 1);
  }
  ...
  Future<void> _goToStep(int target) async {
    ...
    if (target >= 1 && step == 0) { await _continueFromDates(); if (!mounted) return; }
    if (target >= 2 && step == 1) _continueFromPrayers();
  }

  void _changeDateMode(DateSelectionMode mode) { ref.read(calendarControllerProvider.notifier).setSelectionMode(mode); setState(() => prayers.clear()); }
  void _clearDates() { ref.read(calendarControllerProvider.notifier).clear(); setState(() => prayers.clear()); }

  void _continueFromPrayers() {
    final analysis = selectedAnalysis;
    if (prayers.isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Select at least one missed prayer.'))); return; }
    if (analysis.newCount <= 0) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('All selected prayer combinations are already tracked.'))); return; }
    setState(() => step = 2);
  }
  ...
  Future<void> _confirmAndSave() async {
    ...
    try {
      ...
      final before = _availability.analyze(userId: userId, dates: dates, prayerTypes: prayers, existingRecords: existing);
      await ref.read(qazaServiceProvider).recordQazaForDates(userId: userId, dates: dates, prayerTypes: prayers);
      final after = await ref.read(qazaServiceProvider).getRecords(userId: userId);
      final afterAnalysis = _availability.analyze(userId: userId, dates: dates, prayerTypes: prayers, existingRecords: after);
      final created = (before.newCount - afterAnalysis.newCount).clamp(0, before.newCount);
      if (!mounted) return;
      if (created == 0) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Nothing new was added. The selected combinations are already tracked.'))); return; }
      await showDialog<void>(context: context, builder: (context) => AlertDialog(title: const Text('Qaza records created'), content: Text('$created independent Qaza record${created == 1 ? '' : 's'} were added to your ledger. Existing records were preserved.'), actions: [FilledButton(onPressed: () => Navigator.pop(context), child: const Text('Done'))]));
  ...
  Widget _dateStep() {
    ...
    final userId = ref.read(activeUserIdProvider);
    final isDateUnavailable = userId == null ? null : (DateTime date) => !_availability.isDateAvailable(userId: userId, date: date, existingRecords: existing);
    return ListView(key: const ValueKey('qaza_step_list_0'), padding: const EdgeInsets.fromLTRB(20, 12, 20, 28), children: [
      ...
      const SizedBox(height: 12), const _InfoBox(text: 'Gregorian and Hijri dates are shown by the calendar. A date remains selectable when at least one prayer is still eligible.'), const SizedBox(height: 16),
      CalendarPicker(key: const Key('qaza_calendar_picker'), qazaDates: qazaDates, isDateUnavailable: isDateUnavailable), const SizedBox(height: 12),
  ...
  Widget _prayerStep() {
    final allAvailable = PrayerType.values.every((prayer) => _analysisFor([prayer]).newCount > 0);
    final analysis = selectedAnalysis;
    return ListView(key: const ValueKey('qaza_step_list_1'), padding: const EdgeInsets.fromLTRB(20, 12, 20, 28), children: [
      Text('Step 2 of 3 • Select Missed Prayers', style: Theme.of(context).textTheme.labelLarge), const SizedBox(height: 6), Text('Select Missed Prayers', style: Theme.of(context).textTheme.headlineMedium), const SizedBox(height: 8),
      Text('${dates.length} ${dates.length == 1 ? 'date' : 'dates'} selected • Choose the prayers that were missed.', style: Theme.of(context).textTheme.bodyMedium), const SizedBox(height: 16),
      Row(children: [Expanded(child: OutlinedButton.icon(onPressed: checking || saving ? null : () => setState(() { prayers.addAll(PrayerType.values.where((prayer) => _analysisFor([prayer]).newCount > 0)); }), icon: Icon(allAvailable ? Icons.done_all_rounded : Icons.select_all_rounded), label: const Text('Select Available'))), const SizedBox(width: 10), Expanded(child: OutlinedButton.icon(onPressed: prayers.isEmpty || saving ? null : () => setState(prayers.clear), icon: const Icon(Icons.clear_all_rounded), label: const Text('Clear')))]), const SizedBox(height: 12),
  ...
  Widget _reviewStep() {
    ...
    return ListView(key: const ValueKey('qaza_step_list_2'), padding: const EdgeInsets.fromLTRB(20, 12, 20, 28), children: [
      Text('Step 3 of 3 • Review & Add', style: Theme.of(context).textTheme.labelLarge), const SizedBox(height: 6), Text('Review & Add', style: Theme.of(context).textTheme.headlineMedium), const SizedBox(height: 8),
      Text('Review the exact date + prayer combinations before they are added to your Qaza ledger.'), const SizedBox(height: 16),
  ...
  Widget _prayerOption(PrayerType prayer) {
    final analysis = _analysisFor([prayer]); final available = analysis.newCount > 0; final selected = prayers.contains(prayer); final scheme = Theme.of(context).colorScheme;
    final status = analysis.alreadyPrayed > 0 && analysis.newCount == 0 ? 'Already prayed for selected date${dates.length == 1 ? '' : 's'}' : analysis.newCount == 0 ? 'Already tracked for all selected dates' : analysis.newCount == dates.length ? 'Available for all ${dates.length} date${dates.length == 1 ? '' : 's'}' : 'Available for ${analysis.newCount} of ${dates.length} selected dates';
    return Card(margin: const EdgeInsets.only(bottom: 8), child: CheckboxListTile(value: selected && available, onChanged: saving || !available ? null : (value) => setState(() { if (value == true) { prayers.add(prayer); } else { prayers.remove(prayer); } }), secondary: CircleAvatar(backgroundColor: available ? scheme.primaryContainer : scheme.surfaceContainerHighest, child: Icon(prayer.icon, color: available ? scheme.onPrimaryContainer : scheme.onSurfaceVariant)), title: Text(prayer.label), subtitle: Text('$status • ${prayer.rakats}'), controlAffinity: ListTileControlAffinity.trailing));
  }
}
...
class _DateSelectionSummary extends StatelessWidget {
  const _DateSelectionSummary({required this.dates, required this.mode, this.onClear}); final List<DateTime> dates; final DateSelectionMode mode; final VoidCallback? onClear;
  String _formatDate(DateTime date) => '${date.day.toString().padLeft(2, '0')}/${date.month.toString().padLeft(2, '0')}/${date.year}';
  @override Widget build(BuildContext context) { final theme = Theme.of(context); final text = dates.isEmpty ? 'No dates selected' : dates.length == 1 ? 'Selected date: ${_formatDate(dates.first)}' : mode == DateSelectionMode.range ? 'Selected range: ${_formatDate(dates.first)} – ${_formatDate(dates.last)} (${dates.length} days)' : 'Selected dates: ${dates.length}'; return Card(margin: EdgeInsets.zero, child: ListTile(dense: true, leading: const Icon(Icons.event_available_rounded), title: Text(text, style: theme.textTheme.bodyMedium), trailing: onClear == null ? null : IconButton(key: const Key('qaza_clear_dates_button'), tooltip: 'Clear selected dates', onPressed: onClear, icon: const Icon(Icons.clear_rounded)))); }
}
...
class _SummaryRow extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Padding(padding: const EdgeInsets.symmetric(vertical: 4), child: Row(children: [Expanded(child: Text(label)), Text(value, style: emphasis ? Theme.of(context).textTheme.titleMedium : null)]));
}
