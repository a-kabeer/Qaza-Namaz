...
class _AddQazaScreenState extends ConsumerState<AddQazaScreen> {
  ...
  Widget _dateStep() {
    ...
    final isDateUnavailable = userId == null
        ? null
        : (DateTime date) => !availability.isDateAvailable(
              userId: userId,
              date: date,
    ...
    return ListView(
      ...
      children: [
        ...
        Text('Only today and earlier dates can be recorded. Dates are disabled only when no prayer is still available.', style: Theme.of(context).textTheme.bodyMedium),
