...
class _QazaAddFlowV2ScreenState extends ConsumerState<QazaAddFlowV2Screen> {
  ...
  Widget build(BuildContext context) => Scaffold(
        appBar: AppBar(
          ...
          leading: IconButton(tooltip: step == 0 ? 'Close' : 'Back', onPressed: () => step == 0 ? Navigator.pop(context) : setState(() => step--), icon: Icon(step == 0 ? Icons.close_rounded : Icons.arrow_back_rounded)),
        ),
        body: SafeArea(
          child: Column(
            children: [
      ...
      children: [
        ...
        for (final prayer in PrayerType.values)
          Card(
            ...
            child: CheckboxListTile(value: prayers.contains(prayer), onChanged: saving ? null : (value) => setState(() => value == true ? prayers.add(prayer) : prayers.remove(prayer)), secondary: CircleAvatar(child: Icon(prayer.icon)), title: Text(prayer.label), subtitle: Text(prayer.rakats), controlAffinity: ListTileControlAffinity.trailing),
          ),
        const SizedBox(height: 8),
    ...
    return Padding(
      ...
      child: Row(children: [
        for (var i = 0; i < labels.length; i++) ...[
          ...
          CircleAvatar(radius: 14, backgroundColor: i <= step ? scheme.primary : scheme.surfaceContainerHighest, child: Text('${i + 1}', style: TextStyle(color: i <= step ? scheme.onPrimary : scheme.onSurfaceVariant, fontSize: 12, fontWeight: FontWeight.w700))),
