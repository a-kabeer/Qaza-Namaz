...
class QazaAddFlowV2Screen extends ConsumerStatefulWidget {
  ...
  @Deprecated('Use Riverpod provider overrides for tests.')
...
class _QazaAddFlowV2ScreenState extends ConsumerState<QazaAddFlowV2Screen> {
  CalendarEngine get calendarEngine => widget.engine ?? ref.read(calendarEngineProvider);
  QazaService get service => widget.service ?? ref.read(qazaServiceProvider);
  String get userId => widget.userId ?? ref.read(requiredUserIdProvider);
  ...
  int get existingCombinations {
    final keys = existing.map((r) => '${r.prayerType.name}_${calendarEngine.dateKey(r.originalDate)}').toSet();
    return dates.expand((d) => prayers.map((p) => '${p.name}_${calendarEngine.dateKey(d)}')).where(keys.contains).length;
  }
  ...
  Widget _modeStep() => ListView(key: const ValueKey('qaza_step_list_0'), padding: const EdgeInsets.all(20), children: [
        Text('Step 1 of 3 • Range Setup', style: Theme.of(context).textTheme.labelLarge), const SizedBox(height: 6), Text('Add Qaza', style: Theme.of(context).textTheme.headlineMedium), const SizedBox(height: 8), const Text('Choose how you want to identify the missed-prayer dates.'), const SizedBox(height: 22),
        _ChoiceCard(title: 'Calendar', icon: Icons.calendar_month_rounded, child: SegmentedButton<CalendarMode>(segments: const [ButtonSegment(value: CalendarMode.gregorian, icon: Icon(Icons.calendar_today_rounded), label: Text('Gregorian')), ButtonSegment(value: CalendarMode.hijri, icon: Icon(Icons.nightlight_round), label: Text('Hijri'))], selected: {calendarMode}, onSelectionChanged: (v) => setState(() => calendarMode = v.first))),
        const SizedBox(height: 14), _ChoiceCard(title: 'Date selection', icon: Icons.date_range_rounded, child: SegmentedButton<DateSelectionMode>(segments: const [ButtonSegment(value: DateSelectionMode.single, icon: Icon(Icons.today_rounded), label: Text('Single Date')), ButtonSegment(value: DateSelectionMode.range, icon: Icon(Icons.date_range_rounded), label: Text('Date Range'))], selected: {dateMode}, onSelectionChanged: (v) => setState(() { dateMode = v.first; if (dateMode == DateSelectionMode.single) endDate = null; existing = const []; }))),
        const SizedBox(height: 18), const _InfoBox(text: 'Each date + prayer combination becomes one independent Qaza record. Witr remains separate from Isha. Dates are stored in the Gregorian calendar; Hijri mode converts the view (Umm al-Qura).'), const SizedBox(height: 24),
        FilledButton.icon(onPressed: () => setState(() => step = 1), icon: const Icon(Icons.arrow_forward_rounded), label: const Text('Continue')),
      ]);

  Widget _dateStep() => ListView(key: const ValueKey('qaza_step_list_1'), padding: const EdgeInsets.all(20), children: [
        Text('Step 2 of 3 • Date Selection', style: Theme.of(context).textTheme.labelLarge), const SizedBox(height: 6), Text('Choose ${dateMode == DateSelectionMode.single ? 'a date' : 'a date range'}', style: Theme.of(context).textTheme.headlineMedium), const SizedBox(height: 8), const Text('Only today and earlier dates can be recorded. Future dates are locked by the picker.'), const SizedBox(height: 16),
        CalendarPicker(key: const Key('qaza_calendar_picker'), engine: calendarEngine, mode: calendarMode, selectionMode: dateMode, startDate: startDate, endDate: endDate, onSelectionChanged: _onCalendarSelection), const SizedBox(height: 16),
...
class _ProgressHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    const labels = ['Method', 'Dates', 'Review']; final scheme = Theme.of(context).colorScheme;
    return Padding(padding: const EdgeInsets.fromLTRB(20, 12, 20, 8), child: Row(children: [for (var i = 0; i < labels.length; i++) ...[if (i > 0) Expanded(child: Container(height: 2, color: i <= step ? scheme.primary : Theme.of(context).dividerColor)), CircleAvatar(radius: 14, backgroundColor: i <= step ? scheme.primary : scheme.surfaceContainerHighest, child: Text('${i + 1}', style: TextStyle(color: i <= step ? scheme.onPrimary : scheme.onSurfaceVariant, fontSize: 12, fontWeight: FontWeight.w700))), const SizedBox(width: 6), Text(labels[i], style: Theme.of(context).textTheme.labelMedium)]]));
  }
}
class _ChoiceCard extends StatelessWidget {
  const _ChoiceCard({required this.title, required this.icon, required this.child}); final String title; final IconData icon; final Widget child;
  @override Widget build(BuildContext context) => Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Icon(icon), const SizedBox(width: 10), Text(title, style: Theme.of(context).textTheme.titleMedium)]), const SizedBox(height: 14), child])));
}
