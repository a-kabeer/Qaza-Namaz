...
class _CalendarPickerState extends ConsumerState<CalendarPicker> {
  ...
  Future<Set<DateTime>> _defaultMonthLoader(DateTime month) async {
    final userId = ref.read(activeUserIdProvider);
    if (userId == null) return const <DateTime>{};
    return ref.read(qazaServiceProvider).unavailableDatesForMonth(
          userId: userId,
          month: month,
        );
  }

  Future<Set<DateTime>> _loadUnavailableDates(DateTime month) async {
    final loader = widget.loadUnavailableDates ?? _defaultMonthLoader;
    return loader(month);
  }

  bool _isUnavailableDate(DateTime date) {
    if (_loadingAvailability) return true;
  ...
  Future<void> _loadMonthAvailability(DateTime month) async {
    ...
    try {
      final unavailable = await _loadUnavailableDates(normalizedMonth);
      ...
      if (!mounted || requestId != _requestId) return;
      // Fail open rather than incorrectly marking a whole month unavailable.
      setState(() {
        _monthUnavailable = const <DateTime>{};
        _loadingAvailability = false;
      });
  ...
  String _prompt(CalendarSelectionState state) {
    if (_loadingAvailability) return 'Checking prayer availability for this month…';
      ...
      children: [
        Row(children: [
          IconButton(
            ...
            onPressed: canPrevious && !_loadingAvailability ? () => _goMonth(-1) : null,
          ...
          IconButton(
            ...
            tooltip: 'Next month',
            onPressed: canNext && !_loadingAvailability ? () => _goMonth(1) : null,
        ...
        if (_loadingAvailability) const LinearProgressIndicator(key: Key('calendar_availability_loading'), minHeight: 2),
        _DayGrid(
          anchor: _monthAnchor,
          today: _today,
          ...
          isUnavailableDate: _isUnavailableDate,
        ),
        if (selected.isNotEmpty) ...[
          ...
          _SelectionSummary(
            ...
            hijriLabel: _hijriLabel,
            gregorianLabel: (date) => _gregorianLabel(context, date),
            onClear: () => ref.read(calendarControllerProvider.notifier).clear(),
          ),
        ],
      ],
    );
