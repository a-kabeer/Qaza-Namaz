...
class PageScaffold extends StatelessWidget {
  ...
  final String title; final Widget child; final VoidCallback? onBack; final List<Widget> actions;
  @override Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: Text(title), leading: onBack == null ? null : IconButton(tooltip: 'Back', onPressed: onBack, icon: const Icon(Icons.arrow_back_rounded)), actions: actions),
    body: child,
  );
}
...
class PrimaryButton extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => icon == null
...
class IconActionButton extends StatelessWidget {
  ...
  final IconData icon; final String tooltip; final VoidCallback? onPressed; final Color? color;
  @override Widget build(BuildContext context) => IconButton(tooltip: tooltip, onPressed: onPressed, icon: Icon(icon), color: color);
}
...
class StatusChip extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: color ?? scheme.onPrimaryContainer.withValues(alpha: .12), borderRadius: BorderRadius.circular(99)), child: Text(label, style: Theme.of(context).textTheme.labelMedium));
  }
}

class LoadingState extends StatelessWidget {
  const LoadingState({super.key, this.padding = 64, this.message});
  final double padding; final String? message;
  @override Widget build(BuildContext context) => Padding(padding: EdgeInsets.all(padding), child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
...
class ErrorState extends StatelessWidget {
  ...
  final String message; final VoidCallback? onRetry; final String title; final IconData icon;
  @override Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    return Center(child: Padding(padding: const EdgeInsets.all(AppSpacing.xl), child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(height: AppSpacing.lg), Text(title, style: Theme.of(context).textTheme.titleMedium), const SizedBox(height: AppSpacing.sm),
...
class EmptyState extends StatelessWidget {
  ...
  final String title; final String? message; final IconData icon; final Widget? child;
  @override Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    return Center(child: Padding(padding: const EdgeInsets.all(AppSpacing.xl), child: Column(mainAxisSize: MainAxisSize.min, children: [
      Container(width: 64, height: 64, decoration: BoxDecoration(color: scheme.primary.withValues(alpha: .10), borderRadius: BorderRadius.circular(20)), child: Icon(icon, color: scheme.primary)),
      const SizedBox(height: AppSpacing.lg), Text(title, style: Theme.of(context).textTheme.titleMedium),
...
class SettingsSection extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [SectionHeading(title: title, subtitle: subtitle), const SizedBox(height: AppSpacing.sm), Card(child: child)]);
}

class SettingsNavRow extends StatelessWidget {
  const SettingsNavRow({super.key, required this.icon, required this.title, this.subtitle, this.onTap});
  final IconData icon; final String title; final String? subtitle; final VoidCallback? onTap;
  @override Widget build(BuildContext context) => ListTile(leading: Icon(icon), title: Text(title), subtitle: subtitle == null ? null : Text(subtitle!), trailing: const Icon(Icons.chevron_right_rounded), onTap: onTap);
}
...
Future<bool> confirmDestructive(BuildContext context, {required String title, required String message, required String confirmLabel}) async {
  final scheme = Theme.of(context).colorScheme;
  final result = await showDialog<bool>(context: context, builder: (dialogContext) => AlertDialog(title: Text(title), content: Text(message), actions: [
...
class DestructiveActionRow extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    return Card(color: scheme.errorContainer.withValues(alpha: .35), child: ListTile(leading: Icon(icon, color: scheme.error), title: Text(label, style: TextStyle(color: scheme.error, fontWeight: FontWeight.w600)), subtitle: Text(description), onTap: () => _handleTap(context)));
  }
}
...
class AccountSection extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    final theme = Theme.of(context); final scheme = theme.colorScheme; final hasPhoto = user.photoUrl != null && user.photoUrl!.isNotEmpty;
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      Card(child: Padding(padding: const EdgeInsets.all(AppSpacing.lg), child: Row(children: [
        CircleAvatar(radius: 28, backgroundColor: scheme.primaryContainer, foregroundImage: hasPhoto ? NetworkImage(user.photoUrl!) : null, child: Icon(Icons.person_rounded, color: scheme.onPrimaryContainer)), const SizedBox(width: AppSpacing.lg),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(_name, style: theme.textTheme.titleMedium), const SizedBox(height: 2), Text(user.email.isEmpty ? 'Signed in with Google' : user.email, style: theme.textTheme.bodyMedium)])),
      ]))),
      ...
      const SizedBox(height: AppSpacing.lg), Text('Developer context', style: theme.textTheme.titleSmall), const SizedBox(height: AppSpacing.sm),
      Card(child: ListTile(leading: const Icon(Icons.code_rounded), title: const Text('Firebase UID'), subtitle: Text(user.id.isEmpty ? 'Not available' : user.id))), const SizedBox(height: AppSpacing.xl),
...
class ProgressRing extends StatelessWidget {
  ...
  final double progress; final double size; final double strokeWidth;
  @override Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
...
class ProgressOverviewCard extends StatelessWidget {
  ...
  final QazaProgress progress; final Widget? header;
  @override Widget build(BuildContext context) {
    final total = progress.pending + progress.completed; final ratio = total == 0 ? 0.0 : progress.completed / total;
    return Card(child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: AppSpacing.lg), ClipRRect(borderRadius: BorderRadius.circular(99), child: LinearProgressIndicator(value: ratio, minHeight: 10)), const SizedBox(height: AppSpacing.sm), Text(total == 0 ? 'No records yet' : '${(ratio * 100).round()}% completed'),
    ])));
  }
}
...
class PrayerTile extends StatelessWidget {
  ...
  final PrayerType prayer; final String? subtitle; final Widget? trailing; final VoidCallback? onTap;
  @override Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Row(children: [
            CircleAvatar(child: Icon(prayer.icon)), const SizedBox(width: AppSpacing.md),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(prayer.label, style: theme.textTheme.titleMedium), if (subtitle != null) Text(subtitle!, style: theme.textTheme.bodySmall)])),
            if (trailing != null) trailing!, const SizedBox(width: AppSpacing.xs), const Icon(Icons.chevron_right_rounded),
          ]),
        ),
      ),
    );
