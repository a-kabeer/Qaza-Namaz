...
void main() {
  ...
  test('production read paths stay bounded with a 5000-record ledger',
      () async {
    await repository.addRecords([
      for (var i = 0; i < 5000; i++)
        record(
            ...
            date: DateTime(2010, 1, 1).add(Duration(days: i))),
    ]);
    ...
    final oldest = await service.oldestPending(
  ...
  test('history remains paginated and user-scoped', () async {
    ...
    final first = await service.getHistoryPage(
        userId: 'user-a', limit: 50, status: QazaStatus.completed);
  ...
  test('complete-oldest workflow changes only the selected prayer record',
      () async {
    await repository.addRecords([
      record(
          id: 'f-late',
    ...
    final fajr = await repository.getRecords(
        userId: 'user-a', prayerType: PrayerType.fajr);
    final zuhr = await repository.getRecords(
        userId: 'user-a', prayerType: PrayerType.zuhr);
    expect(
        fajr.firstWhere((r) => r.id == 'f-old').status, QazaStatus.completed);
    expect(fajr.firstWhere((r) => r.id == 'f-late').status, QazaStatus.pending);
    expect(zuhr.single.status, QazaStatus.pending);
  });

  test(
      'duplicate protection and account isolation hold across the production repository',
      () async {
    ...
    expect((await repository.getRecords(userId: 'user-a')).single.status,
        QazaStatus.pending);
  });
}
