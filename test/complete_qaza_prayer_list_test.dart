...
void main() {
  ...
  Future<ProviderContainer> pumpPage(
    ...
    Widget home = const CompleteQazaScreen(),
  }) async {
  ...
  group('navigation', () {
    for (final prayer in PrayerType.values) {
      testWidgets('${prayer.name} opens Qaza filtered to it', (tester) async {
        final repository = await ledger();
        // Pushed on top of the workspace, the way the FAB opens it.
        final container =
            await pumpPage(tester, repository, home: const WorkspaceShell());
        await tester.tap(find.byKey(const Key('complete_qaza_fab')));
        await tester.pumpAndSettle();
        expect(find.byType(CompleteQazaScreen), findsOneWidget);

        await tester.tap(row(prayer));
        await tester.pumpAndSettle();

        // The page pops itself, or the tab change would happen out of sight.
        expect(find.byType(CompleteQazaScreen), findsNothing);
