...
void main() {
  QazaRecord record({required String id, required String userId}) {
    ...
    return QazaRecord(id: id, userId: userId, prayerType: PrayerType.fajr, originalDate: date, status: QazaStatus.pending, createdAt: date, updatedAt: date);
  }

  test('setActiveUser does not materialize the full local snapshot', () async {
    final localStore = InMemoryQazaLocalStore();
    final repository = OfflineFirstQazaRepository(remote: InMemoryQazaRepository(), localStore: localStore);
  ...
  test('bounded page read does not materialize the legacy snapshot', () async {
    ...
    addTearDown(repository.dispose);
    await repository.setActiveUser('user-a');
    final page = await repository.getPage(userId: 'user-a', limit: 50, status: QazaStatus.pending);
    expect(page.records, isEmpty);
    expect(localStore.loadCalls, 0);
  });

  test('oldest-pending read does not materialize the legacy snapshot', () async {
    final localStore = InMemoryQazaLocalStore();
    final repository = OfflineFirstQazaRepository(remote: InMemoryQazaRepository(), localStore: localStore);
    addTearDown(repository.dispose);
    await repository.setActiveUser('user-a');
  ...
  test('explicit sync remains the controlled legacy full-ledger entry point', () async {
    final localStore = InMemoryQazaLocalStore();
    final repository = OfflineFirstQazaRepository(remote: InMemoryQazaRepository(), localStore: localStore);
    addTearDown(repository.dispose);
    await repository.setActiveUser('user-a');
