...
void main() {
  ...
  test('migrates all users and preserves completed records', () async {
    ...
    expect(
        (await database.qazaRecordsDao.findById(userId: 'user-b', id: 'b1'))!
            .status,
  ...
  test('deduplicates the same logical prayer without losing completion',
      () async {
    final preferences = await SharedPreferences.getInstance();
    ...
    expect(await database.qazaRecordsDao.count(userId: 'user-a'), 1);
    final migrated =
        (await database.qazaRecordsDao.getPage(userId: 'user-a')).single;
    expect(migrated.status, QazaStatus.completed);
    // Drift stores DateTimes as unix timestamps and reads them back in the
    // host timezone, dropping the isUtc flag. The instant is preserved, so
    // compare UTC components to keep this assertion timezone-independent.
    final completedAt = migrated.completedAt?.toUtc();
    expect(completedAt?.year, 2026);
    expect(completedAt?.month, 2);
    expect(completedAt?.day, 1);
    expect(completedAt?.hour, 0);
    expect(completedAt?.minute, 0);
  });

  test('does not mark migration complete when legacy data is invalid',
      () async {
    final preferences = await SharedPreferences.getInstance();
    await preferences.setString(
        SharedPreferencesToDriftMigrator.legacyStorageKey, '{not-valid-json');
    final database = AppDatabase(NativeDatabase.memory());
    addTearDown(database.close);
    await expectLater(
