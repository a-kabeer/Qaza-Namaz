...
Future<void> pumpScreen(
  ...
  await tester.pumpWidget(ProviderScope(
    ...
    child: const TestApp(home: HistoryProgressScreen()),
  ));
...
void main() {
  testWidgets('filters the ledger by prayer and status', (tester) async {
    ...
    await selectDropdown(
        tester, const Key('history_status_filter'), 'Completed');
  ...
  testWidgets('filters the ledger by original Qaza date range', (tester) async {
    ...
    await repository.addRecords([
      record(
          id: 'inside',
          prayer: PrayerType.fajr,
          originalDate: DateTime(2026, 9, 3)),
      record(
          id: 'outside',
          prayer: PrayerType.zuhr,
    ...
    await tester
        .tap(find.bySemanticsLabel(RegExp(r'Thursday, September 3, 2026')));
    await tester.pumpAndSettle();
    await tester
        .tap(find.bySemanticsLabel(RegExp(r'Saturday, September 5, 2026')));
    await tester.pumpAndSettle();
  ...
  testWidgets('sorts newest original Qaza date first and labels both dates',
      (tester) async {
    final repository = InMemoryQazaRepository();
    await repository.addRecords([
      record(
      ...
      record(
          ...
          originalDate: DateTime(2026, 8, 15)),
    ]);
    await pumpScreen(tester, repository);
    final newFinder = find.text('Asr Qaza');
    final oldFinder = find.text('Fajr Qaza');
    expect(tester.getCenter(newFinder).dy,
        lessThan(tester.getCenter(oldFinder).dy));
    expect(
        find.textContaining('Original Qaza date: 01 Sep 2026'), findsOneWidget);
    expect(find.textContaining('Completed: 11 Sep 2026 09:00'), findsOneWidget);
    expect(
        find.textContaining('Original Qaza date: 01 Aug 2026'), findsOneWidget);
  ...
  testWidgets('loads the next page on scroll and keeps the list lazy',
      (tester) async {
    final repository = InMemoryQazaRepository();
    await repository.addRecords([
      for (var i = 0; i < 120; i++)
        record(
            id: 'item-$i',
            prayer: i.isEven ? PrayerType.fajr : PrayerType.zuhr,
            originalDate: DateTime(2026, 9, 17).subtract(Duration(days: i)),
            status: QazaStatus.completed,
            completedAt: DateTime(2026, 9, 18).subtract(Duration(days: i))),
    ]);
    await pumpScreen(tester, repository);
    expect(repository.historyPageCalls, 1);
    expect(
        find.textContaining('Original Qaza date: 29 Jul 2026'), findsNothing);
    ...
    expect(
        find.textContaining('Original Qaza date: 29 Jul 2026'), findsOneWidget);
  });

  testWidgets('clear filters resets pagination and returns to the first page',
      (tester) async {
    final repository = InMemoryQazaRepository();
    await repository.addRecords([
      for (var i = 0; i < 120; i++)
    ...
    expect(
        find.textContaining('Original Qaza date: 17 Sep 2026'), findsOneWidget);
  });

  testWidgets(
      'refresh requests a new first page without using the loaded ledger provider',
      (tester) async {
    ...
    await repository.addRecords([
      record(
          id: 'newest',
          prayer: PrayerType.fajr,
          originalDate: DateTime(2026, 9, 17),
          status: QazaStatus.completed,
  ...
  test('two simultaneous page requests result in one database request',
      () async {
...
class _HistoryOnlyRepository extends InMemoryQazaRepository {
  ...
  Future<List<QazaRecord>> getRecords(
      {required String userId, PrayerType? prayerType, QazaStatus? status}) {
...
class _SlowHistoryRepository extends InMemoryQazaRepository {
  ...
  Future<QazaHistoryPage> getHistoryPage(
    ...
    final page = await super.getHistoryPage(
        userId: userId,
        limit: limit,
        prayerType: prayerType,
        status: status,
