...
void main() {
  ...
  QazaRecord record(
    ...
    return QazaRecord(id: id, userId: userId, prayerType: prayer, originalDate: date, status: status, completedAt: completedAt, createdAt: date, updatedAt: completedAt ?? date);
  }

  Future<SharedPreferences> prefsWith(Map<String, dynamic> recordsByUser) async {
    SharedPreferences.setMockInitialValues({SharedPreferencesToDriftMigrator.legacyStorageKey: jsonEncode({'recordsByUser': recordsByUser})});
    return SharedPreferences.getInstance();
  }

  test('fresh install with no legacy cache completes safely', () async {
    final prefs = await SharedPreferences.getInstance();
    final result = await SharedPreferencesToDriftMigrator(database: db, preferences: prefs).migrate();
    expect(result.migratedRecordCount, 0);
    expect(prefs.getBool(SharedPreferencesToDriftMigrator.migrationKey), true);
    expect(prefs.getInt(SharedPreferencesToDriftMigrator.migrationVersionKey), SharedPreferencesToDriftMigrator.migrationVersion);
  });

  test('migrates records, deduplicates by prayer/date, and is idempotent', () async {
    final pending = record('pending-id');
    final completed = record('completed-id', status: QazaStatus.completed, originalDate: DateTime.utc(2020, 1, 2), completedAt: DateTime.utc(2026, 1, 1));
    ...
    final prefs = await prefsWith({'user-a': [pending.toJson(), duplicatePending.toJson(), completed.toJson(), duplicateCompleted.toJson()]});
    final migrator = SharedPreferencesToDriftMigrator(database: db, preferences: prefs);
  ...
  test('canonicalizes timezone-aware legacy dates as calendar dates before deduplication', () async {
    ...
    final sameCalendarDate = record('date-b', originalDate: DateTime.utc(2026, 4, 10, 2));
    final prefs = await prefsWith({'user-a': [first.toJson(), sameCalendarDate.toJson()]});
    final result = await SharedPreferencesToDriftMigrator(database: db, preferences: prefs).migrate();
  ...
  test('malformed legacy JSON does not set completion marker', () async {
    SharedPreferences.setMockInitialValues({SharedPreferencesToDriftMigrator.legacyStorageKey: '{invalid-json'});
    final prefs = await SharedPreferences.getInstance();
    expect(() => SharedPreferencesToDriftMigrator(database: db, preferences: prefs).migrate(), throwsStateError);
    expect(prefs.getBool(SharedPreferencesToDriftMigrator.migrationKey), isNot(true));
  });
  ...
  test('unsupported completed marker version is rejected', () async {
    SharedPreferences.setMockInitialValues({SharedPreferencesToDriftMigrator.migrationKey: true, SharedPreferencesToDriftMigrator.migrationVersionKey: 999});
    final prefs = await SharedPreferences.getInstance();
    expect(() => SharedPreferencesToDriftMigrator(database: db, preferences: prefs).migrate(), throwsStateError);
  });

  test('concurrent migration calls serialize and only one performs the import', () async {
    final prefs = await prefsWith({'user-a': [record('concurrent').toJson()]});
    final migrator = SharedPreferencesToDriftMigrator(database: db, preferences: prefs);
    final results = await Future.wait([migrator.migrate(), migrator.migrate(), migrator.migrate()]);
