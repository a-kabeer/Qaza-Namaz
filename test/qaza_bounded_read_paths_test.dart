...
void main() {
  ...
  QazaRecord record(int i,
      {String userId = 'user-a', PrayerType prayer = PrayerType.fajr}) {
    final date = DateTime(2020, 1, 1).add(Duration(days: i));
    return QazaRecord(
        id: '${userId}_${prayer.name}_$i',
        userId: userId,
        prayerType: prayer,
        originalDate: date,
  ...
  test('keyset page remains bounded for a 1001-record ledger', () async {
    ...
    final first = await repository.getPage(
        userId: 'user-a', limit: 50, status: QazaStatus.pending);
    final second = await repository.getPage(
        userId: 'user-a',
        limit: 50,
        status: QazaStatus.pending,
  ...
  test('oldest pending is resolved directly', () async {
    ...
    expect(
        (await repository.getOldestPending(
                userId: 'user-a', prayerType: PrayerType.fajr))
            ?.id,
        'user-a_fajr_1');
  });

  test('service oldest-pending path delegates to bounded repository query',
      () async {
    await repository.addRecords([record(10), record(2), record(7)]);
    expect(
        (await service.oldestPending(
                userId: 'user-a', prayerType: PrayerType.fajr))
            ?.id,
        'user-a_fajr_2');
  });
  ...
  test('aggregate progress is database-backed', () async {
    await repository
        .addRecords([record(1), record(2, prayer: PrayerType.zuhr)]);
    await repository.completeRecord(
        userId: 'user-a',
        recordId: 'user-a_fajr_1',
