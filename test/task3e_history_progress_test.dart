...
void main() {
  testWidgets('Task 3E shows derived overall and six-prayer progress',
      (tester) async {
    ...
    await repository.addRecords([
      ...
      record(
          ...
          originalDate: DateTime(2026, 8, 18),
          status: QazaStatus.completed,
  ...
  testWidgets(
      'Task 3E history sorts by original date and shows pending/completed records',
      (tester) async {
    final repository = InMemoryQazaRepository();
    await repository.addRecords([
      record(
          id: 'old_completed',
          prayer: PrayerType.fajr,
          originalDate: DateTime(2021, 10, 12),
          status: QazaStatus.completed,
          completedAt: DateTime(2026, 9, 2, 15, 45)),
      record(
          id: 'new_completed',
          prayer: PrayerType.zuhr,
          originalDate: DateTime(2021, 10, 11),
          status: QazaStatus.completed,
          completedAt: DateTime(2026, 9, 3, 13, 15)),
      record(
          id: 'pending',
          prayer: PrayerType.asr,
          originalDate: DateTime(2021, 10, 10)),
    ]);
    await pumpScreen(tester, repository);
    ...
    expect(
        find.textContaining('Original Qaza date: 11 Oct 2021'), findsOneWidget);
    expect(
        find.textContaining('Original Qaza date: 12 Oct 2021'), findsOneWidget);
    expect(find.textContaining('Status: Pending'), findsOneWidget);
    expect(tester.getCenter(oldFinder).dy,
        lessThan(tester.getCenter(newFinder).dy));
  });

  testWidgets('Task 3E empty state is explicit when there are no Qaza records',
      (tester) async {
    final repository = InMemoryQazaRepository();
    await pumpScreen(tester, repository);
    expect(find.text('No Qaza records yet.'), findsOneWidget);
    expect(find.textContaining('Your Qaza records will appear here'),
        findsOneWidget);
  });
}
