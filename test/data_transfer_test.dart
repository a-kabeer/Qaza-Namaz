...
void main() {
  ...
  test('exports a valid versioned document', () async {
    ...
    final decoded = jsonDecode(await service.exportJson(
        userId: userA,
        appVersion: appVersion,
        exportedAt: DateTime.utc(2026, 2, 1))) as Map<String, dynamic>;
  ...
  test('round trip preserves ledger semantics', () async {
    ...
    final analysis =
        await service.analyzeImport(jsonText: exported, userId: userB);
  ...
  test('rejects duplicate IDs and duplicate prayer/date combinations',
      () async {
    ...
    expect(
        () => service.analyzeImport(
            jsonText: exportDocument(
                [record(), record(id: 'another-id', prayer: PrayerType.fajr)]),
            userId: userA),
        throwsA(isA<QazaDataTransferException>()));
  });
  ...
  test('re-importing the same file is idempotent', () async {
    ...
    await service.applyImport(
        await service.analyzeImport(jsonText: json, userId: userA));
  ...
  test('existing completed record is preserved against imported pending',
      () async {
    final repo = InMemoryQazaRepository();
    await repo.addRecord(
        record(status: QazaStatus.completed, completedAt: completed));
    final service = QazaDataTransferService(repo);
  ...
  test('invalid record never causes partial writes', () async {
    ...
    await repo.addRecord(record());
    final invalid =
        record(id: 'user-a_zuhr_2026-01-01', prayer: PrayerType.zuhr).toJson()
          ..['status'] = 'not-a-status';
    final valid =
        record(id: 'user-a_asr_2026-01-01', prayer: PrayerType.asr).toJson();
    ...
    expect(
        () => QazaDataTransferService(repo)
            .analyzeImport(jsonText: json, userId: userA),
        throwsA(isA<QazaDataTransferException>()));
