...
const _testUser = AppUser(
    id: 'uid-abc', email: 'kabeer@example.com', displayName: 'Abdul Kabeer');

class _FakeAuthRepository implements AuthRepository {
  int signOutCalls = 0;
  @override
  AppUser? get currentUser => _testUser;
  @override
  Stream<AppUser?> authStateChanges() => Stream<AppUser?>.value(_testUser);
  @override
  Future<AppUser> signInWithGoogle() async => _testUser;
  @override
...
void main() {
  test('Themes use the centralized Stitch design tokens', () {
    ...
    expect(light.colorScheme.secondary, AppTheme.lightSecondary);
    expect(light.colorScheme.tertiaryContainer, AppTheme.tertiaryFixed);
    expect(light.colorScheme.surface, AppTheme.lightBase);
    expect(light.colorScheme.onSurface, AppTheme.lightOnSurface);
    ...
    expect(dark.colorScheme.primaryContainer, AppTheme.darkPrimaryContainer);
    ...
    expect(dark.colorScheme.onSurface, AppTheme.darkOnSurface);
    expect(dark.colorScheme.surfaceContainerLowest, AppTheme.darkSurfaceLowest);
    expect(dark.colorScheme.surfaceContainerLow, AppTheme.darkSurfaceLow);
    expect(dark.colorScheme.surfaceContainer, AppTheme.darkSurface);
    expect(dark.colorScheme.surfaceContainerHigh, AppTheme.darkSurfaceHigh);
    expect(
        dark.colorScheme.surfaceContainerHighest, AppTheme.darkSurfaceHighest);
  ...
  testWidgets('Light theme propagates to the whole app', (tester) async {
    await pumpSettings(tester, ThemeMode.light);
    expect(lastBrightness, Brightness.light);
  });
  testWidgets('Dark theme propagates to the whole app', (tester) async {
    await pumpSettings(tester, ThemeMode.dark);
    expect(lastBrightness, Brightness.dark);
  });
  testWidgets('System theme follows the platform brightness', (tester) async {
    tester.platformDispatcher.platformBrightnessTestValue = Brightness.dark;
    addTearDown(tester.platformDispatcher.clearPlatformBrightnessTestValue);
    await pumpSettings(tester, ThemeMode.system);
    expect(lastBrightness, Brightness.dark);
  });

  testWidgets('Switching theme from Settings propagates globally',
      (tester) async {
    tester.view.physicalSize = const Size(800, 1600);
    tester.view.devicePixelRatio = 1;
    addTearDown(tester.view.resetPhysicalSize);
  ...
  testWidgets('Settings navigation opens Account and lists companion sections',
      (tester) async {
    await pumpSettings(tester, ThemeMode.light);
  ...
  testWidgets('Account screen renders real user identity and sign-out',
      (tester) async {
    ...
    await tester.pumpWidget(ProviderScope(
        ...
        child: TestApp(theme: AppTheme.light(), home: const AccountScreen())));
    await tester.pumpAndSettle();
  ...
  testWidgets('Sign out requires confirmation and never deletes Qaza data',
      (tester) async {
    ...
    expect(
        find.descendant(
            ...
            matching: find.textContaining('NOT deleted')),
        findsOneWidget);
    ...
    final records = await repository.getRecords(userId: 'uid-abc');
    expect(records.length, 2);
    expect(records.every((r) => r.status == QazaStatus.pending), isTrue);
  });
  ...
  testWidgets('Empty, loading and error states render clearly', (tester) async {
    ...
    await tester.pumpWidget(TestApp(
        ...
        home: Scaffold(
          ...
          const Expanded(
              child: LoadingState(message: 'Loading your ledger...')),
          const Expanded(
              child: EmptyState(
                  title: 'No Qaza records yet',
    ...
    expect(find.text('We could not load your Qaza ledger.'), findsOneWidget);
