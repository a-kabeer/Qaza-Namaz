...
void main() {
  ...
  testWidgets('Sign out requires confirmation and never deletes Qaza data', (tester) async {
    ...
    await repository.addRecords([
      QazaRecord(id: 'rec_fajr', userId: 'uid-abc', prayerType: PrayerType.fajr, originalDate: DateTime(2026, 9, 1), createdAt: now, updatedAt: now),
      QazaRecord(id: 'rec_isha', userId: 'uid-abc', prayerType: PrayerType.isha, originalDate: DateTime(2026, 9, 2), createdAt: now, updatedAt: now),
    ]);
    tester.view.physicalSize = const Size(800, 1600); tester.view.devicePixelRatio = 1; addTearDown(tester.view.resetPhysicalSize);
    await tester.pumpWidget(ProviderScope(overrides: [authRepositoryProvider.overrideWithValue(auth)], child: MaterialApp(theme: AppTheme.light(), home: const AccountScreen())));
    await tester.pumpAndSettle();
    ...
    expect(find.text('Sign out?'), findsOneWidget);
    expect(find.descendant(of: find.byType(AlertDialog), matching: find.textContaining('NOT deleted')), findsOneWidget);
    await tester.tap(find.text('Cancel')); await tester.pumpAndSettle(); expect(auth.signOutCalls, 0);
    await tester.tap(find.text('Sign out')); await tester.pumpAndSettle();
    await tester.tap(find.widgetWithText(FilledButton, 'Sign out')); await tester.pumpAndSettle();
