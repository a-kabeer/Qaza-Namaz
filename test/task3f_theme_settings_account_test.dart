...
/// Records the ambient theme of the subtree built under it.
const _testUser = AppUser(
  id: 'uid-abc',
  email: 'kabeer@example.com',
  displayName: 'Abdul Kabeer',
);
...
void main() {
  ...
  testWidgets('Account screen renders real user identity and sign-out', (tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        theme: AppTheme.light(),
        home: AccountScreen(user: _testUser, onSignOut: () async {}),
      ),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets('Empty, loading and error states render clearly', (tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        ...
        home: Scaffold(
          body: Column(
            children: [
              const Expanded(child: LoadingState(message: 'Loading your ledger...')),
              const Expanded(child: EmptyState(title: 'No Qaza records yet', message: 'Add your first record.')),
              Expanded(
                child: ErrorState(
    ...
    // pumpAndSettle would time out here: the indeterminate
    // CircularProgressIndicator in LoadingState animates forever.
    await tester.pump();
    await tester.pump();
