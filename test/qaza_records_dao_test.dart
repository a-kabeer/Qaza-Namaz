...
void main() {
  ...
  test('getPage supports prayer, status and inclusive date filters', () async {
    ...
    expect(
        (await dao.getPendingPage(userId: 'user-a', prayerType: 'fajr'))
            .map((r) => r.id),
        ['f1', 'f3']);
    expect(
        (await dao.getCompletedPage(userId: 'user-a', prayerType: 'fajr'))
            .map((r) => r.id),
        ['f2']);
  });

  test('count, oldest pending and user isolation are database-level queries',
      () async {
    await dao.insertRecords([
      record(
          id: 'later',
    ...
    expect(
        (await dao.getOldestPending(userId: 'user-a', prayerType: 'zuhr'))!.id,
        'zuhr');
  ...
  test('rejects invalid page sizes, offsets and date ranges', () {
    ...
    expect(
        () => dao.getPage(userId: 'user-a', limit: 501), throwsArgumentError);
    expect(
        () => dao.getPage(userId: 'user-a', offset: -1), throwsArgumentError);
    expect(
        () => dao.getPage(
            userId: 'user-a',
  ...
  test(
      'history first page is descending and reports hasMore using limit plus one',
      () async {
    await dao.insertRecords([
      record(
          id: 'a',
    ...
    final page =
        await dao.getHistoryPage(userId: 'user-a', limit: 3, status: null);
  ...
  test('history cursor continues descending order without duplicates',
      () async {
    await dao.insertRecords([
      record(
          id: 'a',
    ...
    final first =
        await dao.getHistoryPage(userId: 'user-a', limit: 2, status: null);
    final second = await dao.getHistoryPage(
        userId: 'user-a',
        limit: 2,
        status: null,
  ...
  test('history remains deterministic when original dates are identical',
      () async {
    await dao.insertRecords([
      record(
          id: 'id-001',
    ...
    final next = await dao.getHistoryPage(
        userId: 'user-a',
        limit: 2,
        status: null,
        beforeOriginalDate: page.nextOriginalDate,
        beforeId: page.nextId);
    expect(next.records.map((r) => r.id), ['id-001']);
    expect(next.hasMore, isFalse);
  });

  test('history applies combined prayer, status and date filters in SQLite',
      () async {
    await dao.insertRecords([
      record(
          id: 'f-old',
    ...
    final page = await dao.getHistoryPage(
        userId: 'user-a',
        limit: 10,
        prayerType: 'fajr',
        status: 'completed',
  ...
  test('history rejects partial cursor, invalid sizes and invalid range', () {
    expect(() => dao.getHistoryPage(userId: 'user-a', beforeId: 'id'),
        throwsArgumentError);
    expect(
        () => dao.getHistoryPage(
            userId: 'user-a', beforeOriginalDate: DateTime(2026, 1, 1)),
        throwsArgumentError);
    expect(() => dao.getHistoryPage(userId: 'user-a', limit: 0),
        throwsArgumentError);
    expect(() => dao.getHistoryPage(userId: 'user-a', limit: 501),
        throwsArgumentError);
    expect(
        () => dao.getHistoryPage(
            userId: 'user-a',
  ...
  test('history stays bounded with 10000 records', () async {
    await dao.insertRecords([
      for (var i = 0; i < 10000; i++)
        record(
            id: 'record-${i.toString().padLeft(5, '0')}',
            userId: 'user-a',
            prayerType: i.isEven ? 'fajr' : 'zuhr',
            date: DateTime(1999, 1, 1).add(Duration(days: i)),
            status: i % 3 == 0 ? 'completed' : 'pending'),
    ]);
    final stopwatch = Stopwatch()..start();
    final page =
        await dao.getHistoryPage(userId: 'user-a', limit: 50, status: null);
    stopwatch.stop();
